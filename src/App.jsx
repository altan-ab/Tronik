...
*/
  ...
  function handleMouseDown(event) {
    const targetKey = event.target.getAttribute('data-note')
    if (!targetKey) return // Eğer synth key değilse çık

    setSynthKeys((prevKeys) =>
      prevKeys.map((key) =>
        key.keyName === targetKey ? { ...key, active: true } : key
      )
    )
  }
  ...
  function handleKeyUp(event) {
    ...
    setSynthKeys((prevKeys) =>
      prevKeys.map((key) =>
        key.computerKey === targetKey ? { ...key, keyPressed: false } : key
      )
    )
  }
  ...
  return (
    <div className="wrapper">
      <div
        ...
        onClick={(e) => console.log(e.target)} // data-note console'da görüldü :)
